...
char msg[2*MAXLINE];    /* for whenever we need to compose an error message */
                        /* this needs to be larger than MAXLINE because some
			   error messages contain paths that are only limited
			   by MAXLINE. */
...
/* A speed measurement routine that runs code twice using Callgrind,
   instead of multiple times collecting wall-clock time. */
double fsecs_callgrind(fsecs_test_funct f, void *argp);
...
int main(int argc, char **argv)
{
    ...
    int use_cgrind = 0;  /* If set, support callgrind measurement (-c) */
     ...
     */
    while ((c = getopt(argc, argv, "f:t:hvVgacl")) != EOF) {
        switch (c) {
	case 'g': /* Generate summary info for the autograder */
	    ...
	    /* This code use to set the trace directory to "./", which
	       was correct for relative paths but broke absolute
	       paths. Using no directory prefix is more like the
	       standard Unix behavior, and allows eiher relative or
	       absolute paths. */
	    strcpy(tracedir, "");
	...
	case 't': /* Directory where the traces are located */
        ...
        case 'a': /* Don't check team structure */
            ...
            break;
#ifdef USE_CALLGRIND
	case 'c': /* Run only twice and use client reqs, for callgrind*/
	    use_cgrind = 1;
	    break;
#endif
	...
	for (i=0; i < num_tracefiles; i++) {
	    ...
	    if (verbose > 1)
		fl_puts("Checking libc malloc for correctness, ");
	    if (use_cgrind) {
		...
		if (verbose > 1)
		    fl_puts("and performance");
		if (use_cgrind)
		    libc_stats[i].secs =
			fsecs_callgrind(eval_libc_speed, &speed_params);
		else
	...
	if (verbose) {
	    ...
	    libc_kops = printresults(num_tracefiles, libc_stats);
	}

	/* In callgrind mode only, skip the student code under -l */
	if (use_cgrind)
	    exit(0);
    }
	...
	if (verbose > 1)
	    fl_puts("Checking mm_malloc for correctness, ");
	if (use_cgrind) {
	    /* Skip correctness check */
	    mm_stats[i].valid = 1;
	} else {
	...
	if (mm_stats[i].valid) {
	    ...
	    if (use_cgrind)
		mm_stats[i].secs =
		    fsecs_callgrind(eval_mm_speed, &speed_params);
	    else
     ...
     */
    if (errors == 0) {
	if (libc_kops*1000 >= 0.9*AVG_LIBC_THRUPUT &&
	    libc_kops*1000 <= 1.1*AVG_LIBC_THRUPUT) {
	    avg_mm_throughput = ops/secs;

	    p1 = UTIL_POINTS * avg_mm_util;
	    if (avg_mm_throughput > AVG_LIBC_THRUPUT) {
		p2 = THRU_POINTS;
	    }
	    else {
		p2 = ((double)THRU_POINTS) *
		    (avg_mm_throughput/AVG_LIBC_THRUPUT);
	    }

	    perfindex = p1 + p2;
	    printf("Estimated perf index = "
		   "%.0f/%d (util) + %.0f/%d (thru) = %.0f/%d\n",
		   p1, UTIL_POINTS,
		   p2, THRU_POINTS,
		   perfindex,
		   UTIL_POINTS + THRU_POINTS);
	} else {
	    p1 = UTIL_POINTS * avg_mm_util;
	    perfindex = 0;

	    printf("Skipping throughput estimate, libc performance is %s\n",
		   (libc_kops ? "outside expected range" : "missing"));
	    printf("Space utilization score: %0.f/%d\n",
		   p1, UTIL_POINTS);
	}
    }
...
{
#ifdef USE_CALLGRIND
    /* The first run warms the (simulated) cache, corresponding to the
       way wall-clock runs use repetition. */
    f(argp);
    ...
    return 1.0;
#else
    /* Really, this shouldn't be called without USE_CALLGRIND */
    return 0.0;
#endif
}
...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if (verbose > 1) {
	printf("Reading tracefile: %s...", filename);
	fflush(stdout);
    }
    ...
    num_read = fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
    assert(num_read == 1);
    num_read = fscanf(tracefile, "%d", &(trace->num_ids));
    assert(num_read == 1);
    num_read = fscanf(tracefile, "%d", &(trace->num_ops));
    assert(num_read == 1);
    num_read = fscanf(tracefile, "%d", &(trace->weight));        /* not used */
    assert(num_read == 1);
	...
	switch(type[0]) {
	case 'a':
	    num_read = fscanf(tracefile, "%u %u", &index, &size);
	    assert(num_read == 2);
	...
	case 'r':
	    ...
	    break;
	case 'f':
	    num_read = fscanf(tracefile, "%ud", &index);
	    assert(num_read == 1);
...
/*
 * fl_puts - Print a message immediately to standard output
 */
void fl_puts(char *msg) {
    fputs(msg, stdout);
    fflush(stdout);
}
...
#ifdef USE_CALLGRIND
    fprintf(stderr, "\t-c         Mode for running under Callgrind.\n");
#endif
